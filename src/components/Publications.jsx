      ...
      <div className="space-y-4">
        {publications.map((p, i) => (
          <motion.div
            ...
            className="flex gap-4 rounded-xl border border-neutral-200 bg-white p-6 transition-all hover:border-emerald-300 hover:shadow-lg hover:shadow-emerald-100/50"
          >
            ...
            <div className="min-w-0">
              ...
              <p className="mt-2 text-sm text-neutral-500">
                <span className="font-medium text-emerald-600">{p.venue}</span>
                {p.volume && ` ${p.volume}`}
                {p.year && ` (${p.year})`}
                {p.page && `, p. ${p.page}`}
              </p>
